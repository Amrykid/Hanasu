...
namespace Hanasu.Core.Songs.Album_Info_Data_Source
{
    public class YesAsia : IAlbumInfoDataSource
    {
        public bool GetAlbumInfo(ref SongData song)
        {
            try
            {
                ...
                var albumtxt = Regex.Replace(Regex.Replace(HtmlTextUtility.Decode(Regex.Match(m.Value, "alt=\".+?\"", RegexOptions.Singleline | RegexOptions.Compiled).Value.Substring(5).Trim('\"')),
                ...
                if (song.AlbumCoverUri.Segments.Last().StartsWith("s_"))
                    song.AlbumCoverUri = new Uri(imgurl.Replace("/s_", "/l_")); //Grabs the larger version.
